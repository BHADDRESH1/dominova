...
export const DashboardLayout = ({ children, role }: DashboardLayoutProps) => {
  ...
  const navItems = {
    owner: [
      ...
      { label: "Policies", href: "/dashboard/policies?role=owner", icon: FileText },
      { label: "Settings", href: "/dashboard/settings", icon: Settings },
    ],
    admin: [
      { label: "Dashboard", href: "/dashboard/admin", icon: LayoutDashboard },
      { label: "Ambassadors", href: "/dashboard/ambassadors?role=admin", icon: Users },
      { label: "Verifications", href: "/dashboard/verifications", icon: FileCheck },
      { label: "Offer Letters", href: "/dashboard/offer-letters", icon: Award },
      { label: "Employees", href: "/dashboard/employees?role=admin", icon: Briefcase },
      { label: "Messages", href: "/dashboard/messages?role=admin", icon: MessageSquare },
      { label: "Policies", href: "/dashboard/policies?role=admin", icon: FileText },
    ],
    ambassador: [
      { label: "Dashboard", href: "/dashboard/ambassador", icon: LayoutDashboard },
      ...
      { label: "Messages", href: "/dashboard/messages?role=ambassador", icon: MessageSquare },
      { label: "Policies", href: "/dashboard/policies?role=ambassador", icon: FileText },
    ],
    employee: [
      { label: "Dashboard", href: "/dashboard/employee", icon: LayoutDashboard },
      { label: "Messages", href: "/dashboard/messages?role=employee", icon: MessageSquare },
      { label: "Policies", href: "/dashboard/policies?role=employee", icon: FileText },
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-sidebar border-b border-sidebar-border h-16 flex items-center justify-between px-4">
        <Link to="/" className="flex items-center gap-2">
          <img src="/dominova_logo.png" alt="Dominova" className="w-8 h-8 object-contain" />
          <span className="text-lg font-bold text-sidebar-foreground">Dominova</span>
        </Link>
        <button
          ...
          <div className="p-4 border-b border-sidebar-border">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-sidebar-accent">
              <div className={cn(
                ...
                config.color === 'ambassador' && "bg-ambassador/20 text-ambassador"
              )}>
